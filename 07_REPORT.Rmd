...
-   Removed prepositions (e.g., “with”, “or”, “for”, “from”, “in”) and adjectives (done using NLP annotation with UDPipe to add part‑of‑speech tags)
...
For fun, I also created histograms of the top 5 ingredients in each country after removing the top 10 ingredients like salt and sugar. The top 5 ingredients made intuitive sense to me but I could also see that in the recipes included in this dataset are not representative of an entire cuisine, but rather more popular recipes. One thing that did surprise me was mozzellera cheese being the number 1 ingredient in Columbian recipes.
...
- Note: at first, I ran `prcomp` and it took about 670 PCs to capture 80% variance (but `prcomp_irlba` would not finish running with that many PCs, given how computationally expensive it was).
...
### t-SNE Embedding
- Applied t-SNE on the PCA results to project recipes into two dimensions
...
```
# Discussion
To see if we can visualize recipes with similar ingredient profiles, we used PCA to reduce dimensionality and applied t-SNE to visualize local neighborhoods. You can see that Asian recipes, particularly, Southern, Eastern, and South-east Asian, are located near each other. You can also see a couple of small North American and South American clusters. European recipes are spread out. One thing I did notice is that sweeter recipes were located by one another, additionally so were cocktail/drink recipes. I think we are seeing clusters by types of meals like desserts, bakery items, etc, rather than geographic region. If that is the case, it might explain why we see Asian recipes together because there was more homogeneity in the type of recipes when I went read through the names. Most of them were savory dinner recipes. The other cuisines, particular those in Europe, seem to have more heterogeneity with both savory and sweet dishes. 

One thing I learned through this process is that food often crosses borders and its important to recognize the impact of colonization, immigration,and globalization. I believe we see a little bit of this in the t-SNE plot as countries overlapped, but I think having a larger sample size, a wider variety of recipes within cuisines, and stronger ingredient standardization to account for words that are different but mean the same (scallions and green onions, for example) would make conclusions stronger.I would also want to ensure that recipes are not American-ized versions and accurately represent traditional cuisine. 


# Next Steps
For next steps, I'd like to do the following:

-   Web scrape more recipes to get a better variety and representation of data and add meal as a categorical variable
...
-   Use a clustering method to identify clusters and describe them
